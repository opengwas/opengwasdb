...
processes so peak RSS is attributable to one workload.
    ...
    pixi run -e dev python benchmarks/benchmark_resolver_evidence_scan.py \
      ...
      --cores 64 --output docs/benchmark-output/opengwasdb_resolver_evidence_scan_v2.json
"""
...
def _resolve_with_limit(
    ...
    return {
        "limit": None if limit is None else limit.as_fingerprint(),
        ...
        "ancestry_rows_read": resolution.diagnostics.ancestry_rows_read,
        "ancestry_stop_reason": resolution.diagnostics.ancestry_stop_reason.value,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPO_ROOT / "docs/benchmark-output/opengwasdb_resolver_evidence_scan_v2.json",
    )
    return parser
...
def _render_markdown(payload: dict[str, Any], manifest: list[AnalysisRow]) -> str:
    ...
    decision = study["decision"]
    scan_limit_version = payload.get("scan_limit_version", 1)

    historical_banner: list[str] = []
    if scan_limit_version == 1:
        historical_banner = [
            "",
    ...
    out: list[str] = [
        ...
        *historical_banner,
        ...
        f"- Scan limit version: {scan_limit_version}",
