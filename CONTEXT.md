...
**Staged Release**:
A Store Release under construction, held at its own unique `.{name}.tmp.{invocation}` sibling of its eventual path until every artifact is written; two invocations for one destination therefore never delete or corrupt each other's work. Committing renames it into place atomically under an advisory lock on the destination's parent directory, which re-checks the destination so a no-overwrite build that loses a race fails loudly rather than replacing the winner (ADR 0043). An error or interruption discards that invocation's own temporary directory and leaves the destination path exactly as it was found — a build or completion run can never leave a half-written release at its real path.
...
**Format Version**:
The version of the OpenGWASDB storage contract required to interpret a Store Release. It describes representation compatibility, not the version of the biological data. It is semantic versioning, `MAJOR.MINOR.PATCH`, and the leftmost non-zero component is the one an incompatible change moves. The current version is `0.1.0`, which is a reset rather than a fifth version: the pre-release formats `0.1`, `1.0`, `2.0` and `3.0` are refused rather than decoded, and the three-component shape is what makes a pre-reset stamp impossible to misread as a current one (ADR 0041). A `3.0` release holds the same bytes and may be restamped; every earlier one is a different encoding and is rebuilt.
