...
| `bin/gate.py` | not a check: runs every gate the config names; `--strict` for CI, `--hook` and `--guard` for an agent's hooks, `--changed` scopes the heavy gates to files changed against the top-level `base_ref` (then the PR base/default-branch fallback when absent), `--stats` for what those two did (each firing is a line in `quality/.events.jsonl`, gitignored; a refused call records its target, an allowed one only the tool; `"events": false` in the config turns it off) | — |
