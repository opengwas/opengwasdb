...
try:
    ...
    write(os.path.join(repo, "quality.json"), json.dumps({"base_ref": "dev",
                                                          "escapes": {"roots": ["src"], "languages": ["python"], "baseline": "escapes-baseline.json"},
    ...
    check("--changed uses the configured base, not the default branch's merge-base", "changed: 1 file(s)" in out, out + err)
